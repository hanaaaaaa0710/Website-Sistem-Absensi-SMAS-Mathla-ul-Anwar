        ...
        <form action="{{ isset($absensi) ? route('absensi-harian.update',$absensi->id) : route('absensi-harian.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select name="siswa_id" class="form-control" required>
                    @foreach($siswa as $s)
                        <option value="{{ $s->id }}" {{ (isset($absensi) && $absensi->siswa_id==$s->id) ? 'selected' : '' }}>
                            {{ $s->nama_siswa ?? '-' }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                <label>Tanggal</label>
                <input type="date" name="tanggal" class="form-control" value="{{ $absensi->tanggal ?? $tanggal->toDateString() }}" required>
            </div>
